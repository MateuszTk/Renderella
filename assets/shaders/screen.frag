...
void main() {
	...
	if (depthValue < 1.0) {
		...
		if (lightData.w >= SPECULARITY_THRESHOLD) {
			vec4 ssrUV = ssr(worldPixelPos).xxyy;
			ivec2 ssrUVLow = ivec2(ssrUV.yw * (256.0 * 255.0)) % 256;
			ssrUV.yw = ssrUVLow;
			ivec2 ssrUVHigh = ivec2(ssrUV.xz * 255.0);
			ssrUV.xz = ssrUVHigh;			

			FragColor = ssrUV / 255.0;
		}
		else {
			FragColor = vec4(0.0);
		}
